...
*
 * @param Party Height
 * @desc Formula for the gauge height for the player party.
 * @default 18
 *
...
Yanfly.Param.PLGPartyHeight = Number(Yanfly.Parameters['Party Height']);
...
Yanfly.Param.PLGPartyColor3 = Number(Yanfly.Parameters['Party Gauge Color 3']);
...
Game_Party.prototype.partyLimitGaugeColor3 = function() {
    return Yanfly.Param.PLGPartyColor3;
};
...
Game_Troop.prototype.partyLimitGaugeColor3 = function() {
    return Yanfly.Param.PLGTroopColor1;
};
...
Window_Base.prototype.drawPartyLimitValue = function(unit, x, y, w, h) {
    ...
    if ($gameParty.partyLimitGauge() === $gameParty.partyLimitGaugeMax()) {
        this.drawText('Max', x, y, size, 'center');
    } else {
...
Window_Base.prototype.drawPartyLimitGauge = function(unit, x, y, w) {
    ...
    var height = Yanfly.Param.PLGPartyHeight;
  //  var fill = $gameParty.partyLimitGauge();
   // var max = $gameParty.partyLimitGaugeMax();
    if ($gameParty.partyLimitGauge() === $gameParty.partyLimitGaugeMax()) {
  //      var c1 = this.textColor(unit.partyLimitGaugeColor2());
        var c1 = this.textColor(14);
        var c2 = this.textColor(18);
        var gw = Math.floor(this.width);
    } else {
   ...
   // var c2 = this.textColor(unit.partyLimitGaugeColor2());
    ...
    if (gw >= 5) {
      for (var i = 0; i < gauges; ++i) {
        var rate = rates[i] || 0;
        this.drawGaugeEx(x, y, gw, height, rate, c1, c2);
        x += gw;
      }
    } else {
      var rate = unit.partyLimitGaugeRate();
      this.drawGaugeEx(x, y, w, height, rate, c1, c2);
    }
};
