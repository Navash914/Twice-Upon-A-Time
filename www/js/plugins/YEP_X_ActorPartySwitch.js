 ...
 *
 * @param Switch Cost
 * @desc PLG Cost of Switching Actor.
 * @default 100
 *
 * @param Show Command
...
Yanfly.Param.PartySwitchCost = String(Yanfly.Parameters['Switch Cost']);
Yanfly.Param.PartySwitchCost = eval(Yanfly.Param.PartySwitchCost);
...
Window_ActorCommand.prototype.isActorPartySwitchEnabled = function() {
  var cost = Yanfly.Param.PartySwitchCost;
  if ($gameParty.partyLimitGauge() < cost) return false;
...
Scene_Battle.prototype.postPartySwitch = function(index, swapOut, swapIn) {
  ...
  if (Imported.YEP_X_PartyLimitGauge) {
      var cost = Yanfly.Param.PartySwitchCost;
      $gameParty.losePartyLimitGauge(cost);
  }
