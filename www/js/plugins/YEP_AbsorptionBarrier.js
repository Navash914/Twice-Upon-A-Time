...
Game_Action.prototype.gainBarrierBonus = function(target, isTarget) {
    ...
    for (var i = 0; i < length; ++i) {
      ...
      value += target.gainBarrierEval(evalBarriers[i] || '', t, this.subject(),
        target);
      if (value > 0) {
        var shieldPower = this.subject().pha;
        var shieldEffect = target.rec;
        value *= shieldPower * shieldEffect;
        target.gainBarrier(value, t);
        value = Math.floor(value);
        if (this.subject().isActor()) this.subject().increaseShieldingDone(value);  // Own Code
        if (target.isActor()) target.increaseShieldingReceived(value); // Own Code
      } else if (value < 0) {
...
Window_Base.prototype.drawActorHp = function(actor, wx, wy, ww) {
    ww = ww || 186;
    var color1 = this.hpGaugeColor1(actor.hpRate());
    var color2 = this.hpGaugeColor2(actor.hpRate());
