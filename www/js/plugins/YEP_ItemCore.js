 ...
 * @default true
 *
 * @param Draw Parameter Rectangles
 * @desc Enabling this will change Scene Item's visual appearance.
 * NO - false     YES - true (recommended)
 * @default true
 *
 ...
 * @default true
 *
 * @param Draw Icon Rectangle
 * @desc Draw the icon rectangle in the status window?
 * NO - false     YES - true
 * @default true
 *
 * @param Icon Size
...
Yanfly.Param.ItemParamRects = String(Yanfly.Parameters['Draw Parameter Rectangles']);
Yanfly.Param.ItemParamRects = eval(Yanfly.Param.ItemParamRects);
...
Yanfly.Param.ItemDrawIconRect = String(Yanfly.Parameters['Draw Icon Rectangle']);
Yanfly.Param.ItemDrawIconRect = eval(Yanfly.Param.ItemDrawIconRect);
...
ItemManager.randomizeInitialStats = function(baseItem, newItem) {
    ...
    for (var i = 0; i < 8; ++i) {
      if (newItem.params[i] === 0) continue;
      newItem.params[i] += Math.floor((Math.random() * randomValue - offset) * baseItem.params[i]);
      if (!Yanfly.Param.ItemNegVar && baseItem.params[i] >= 0) {
        newItem.params[i] = Math.max(newItem.params[i], 0);
      }
    }
...
Window_ItemStatus.prototype.refresh = function() {
    this.contents.clear();
    if (Yanfly.Param.ItemParamRects) this.drawDarkRectEntries();
...
Window_ItemStatus.prototype.drawDarkRectEntries = function() {
    ...
    if (Yanfly.Param.ItemShowIcon) {
      ...
      if (Yanfly.Param.ItemDrawIconRect) this.drawDarkRect(rect.x, rect.y, rect.width, rect.height);
...
Window_ItemStatus.prototype.getItemIcons = function(i, array) {
    this._item.effects.forEach(function(effect) {
      ...
      if (i === 6 && effect.code === Game_Action.EFFECT_ADD_BUFF) {
        var icon = Yanfly.Param.BSCBuffIconStart + effect.dataId;
        array.push(icon);
      }
      if (i === 6 && effect.code === Game_Action.EFFECT_ADD_DEBUFF) {
        var icon = Yanfly.Param.BSCDebuffIconStart + effect.dataId;
        array.push(icon);
      }
...
Window_ItemInfo.prototype.refresh = function() {
    ...
    var dy = 0;
    var item = this._item;
    if ($gameVariables.value(17) === 1) {
  //    this.drawScrollImage();
      this.drawPreItemInfo(dy);
    }
...
Window_ItemInfo.prototype.drawPreItemInfo = function(dy) {
    ...
    var color = this.textColor(17);
    this.changeTextColor(color);
    this.drawText('Passives', dx, dy, dw, 'center');
    return dy + this.lineHeight() + 15;
};
...
Window_ItemInfo.prototype.drawScrollImage = function() {
        var w = this.textPadding() * 2;
        var w2 = this.contents.width - w;
        var h = this.lineHeight() - 14;
  //      var p = 24;
 //       var q = p/2;
    
        var leftArrowSprite = new Sprite(Bitmap.load('img/system/LeftScroll.png'));
        var rightArrowSprite = new Sprite(Bitmap.load('img/system/RightScroll.png'));
    
        leftArrowSprite.anchor.x = 0;
        leftArrowSprite.anchor.y = 0;
        rightArrowSprite.anchor.x = 1;
        rightArrowSprite.anchor.y = 0;
    
        leftArrowSprite.move(w, h);        
        rightArrowSprite.move(w2 + 36, h);

  //      var lScroll = Bitmap.load('img/system/LeftScroll.png');
  //      var rScroll = Bitmap.load('img/system/RightScroll.png');

        this.addChild(leftArrowSprite);
        this.addChild(rightArrowSprite);
  //      this.leftArrowSprite.move(q, h-q-29);

  //      this.rightArrowSprite.bitmap = rScroll;
  //      this.rightArrowSprite.move(w-q-98, h-q-29);
