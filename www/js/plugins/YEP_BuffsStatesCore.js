 ...
 * @default 16
 *
 * @param Buff Font Size
 * @desc The default font size used for turn count for buffs/debuffs.
 * Default: 28
 * @default 16
 *
 ...
 * @default false
 *
 * @param Buff Icon Start
 * @desc Shows the buff/debuff rate for buffs and debuffs.
 * YES - true     NO - false
 * @default 32
 *
 * @param Debuff Icon Start
 * @desc Shows the buff/debuff rate for buffs and debuffs.
 * YES - true     NO - false
 * @default 48
 *
...
Yanfly.Param.BSCBuffFontSize = String(Yanfly.Parameters['Buff Font Size']);
...
Yanfly.Param.BSCBuffIconStart = Number(Yanfly.Parameters['Buff Icon Start']);
Yanfly.Param.BSCDebuffIconStart = Number(Yanfly.Parameters['Debuff Icon Start']);
...
Game_BattlerBase.prototype.buffIconIndex = function(buffLevel, paramId) {
    if (buffLevel > 0) {
      var level = Math.min(buffLevel - 1, 3);
      return Yanfly.Param.BSCBuffIconStart + (level) * 8 + paramId;
    } else if (buffLevel < 0) {
      var level = Math.min(-buffLevel - 1, 3);
      return Yanfly.Param.BSCDebuffIconStart + (level) * 8 + paramId;
    } else {
      return 0;
    }
};
...
Sprite_StateIcon.prototype.drawBuffTurns = function(paramId) {
    ...
    var wh = Window_Base.prototype.lineHeight.call(this);
    var contents = this._turnCounterSprite.bitmap;
    contents.fontSize = Yanfly.Param.BSCBuffFontSize;
...
Sprite_StateIcon.prototype.drawBuffRate = function(paramId) {
    ...
    var wh = Window_Base.prototype.lineHeight.call(this);
    var contents = this._turnCounterSprite.bitmap;
    contents.fontSize = Yanfly.Param.BSCBuffFontSize * 0.75;
...
Window_Base.prototype.drawBuffTurns = function(actor, paramId, wx, wy) {
  ...
  this.changePaintOpacity(true);
  this.contents.fontSize = Yanfly.Param.BSCBuffFontSize;
