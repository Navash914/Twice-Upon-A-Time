...
Window.prototype._refreshArrows = function() {
    ...
    if (Moogle_X.EQL.useScrollImages) {
        ...
        this._Moogle_X_leftArrowSprite.move(q + leftOffsetX, h2 + leftOffsetY);

        this._Moogle_X_rightArrowSprite.bitmap = rightScroll;
        this._Moogle_X_rightArrowSprite.move(w-q-98 + rightOffsetX, h2 + rightOffsetY);
...
Window_Base.prototype.MoogleXrightArrowVisible = function(sceneIndex, scenePages) {
    if (SceneManager._scene instanceof Scene_Item) return false;
    if (this.customMoogleXhArrowsHide()) return false;
    if (!scenePages) return false;
    if (scenePages === 1) return false;
...
Window_EqlVictoryAp.prototype.drawGainedSkills = function(actor, rect) {
    ...
    for (var i = 0; i < actor.eqlNewSkill().length; ++i) {
        ...
        var text2 = '+ More';
        var ww2 = this.textWidth(text2);
        var wx2 = rect.x + (rect.width - ww2) / 2;
        var width = rect.width;
        var color = this.textColor(17);
        this.changeTextColor(color);
        this.drawText(text2, wx2, wy - this.lineHeight()/2, width, 'left');
        ...
        if (!skill) continue;
        var text = '\\c[17]' + '\\i[' + skill.iconIndex + ']' + skill.name + '\\c[0]' + '!';
...
Window_EqlAbilityList.prototype.drawAbilityList = function(item, x, y, w) {
    ...
    for (var i = 0; i < skillList.length; i++) {
        ...
        var ability = $dataSkills[skillList[i]];
        if (ability) {
            this.eqlDrawIcon(ability, x + 8, dy + 2);
...
//    if (rate === 1) return;
//    this.drawGauge(dx, y, width - dx - iconWidth - 2, rate, color1, color2);
//  (x, y, width, rate, color1, color2)
    var x2 = this.standardPadding();
    y = y + 15;
    if (rate !== 1) {
    var color1 = this.textColor(this.processGaugeColor1(rate));
    var color2 = this.textColor(this.processGaugeColor2(rate));     
    this.drawGaugeEx(x2, y + this.lineHeight(), this.contents.width - iconWidth - 2, 12, rate, color1, color2);
    this.changeTextColor(this.textColor(Moogle_X.EQL.apTextColor));
    this.drawText(Moogle_X.EQL.apVocab, x2 + Moogle_X.EQL.apTextOffsetX, y + this.lineHeight(), width);
    }
    this.changeTextColor(this.normalColor());
    this.drawApNumbers(ability, dx, y + this.lineHeight(), width - dx - iconWidth - 2);
    this.drawMasteryIcon(ability, width - iconWidth + 2, y + this.lineHeight() + 2);
};

Window_EqlAbilityList.prototype.processGaugeColor1 = function(rate) {
    var colorId = 0;
    if (rate >= 0.8) {
        colorId = 14; // Yellow
    } else if (rate >= 0.6) {
...
Window_EqlAbilityList.prototype.processGaugeColor2 = function(rate) {
    var colorId = 0;
    if (rate >= 0.8) {
        colorId = 18; // Red
    } else if (rate >= 0.6) {
...
Window_EqlAbilityList.prototype.drawApNumbers = function(ability, x, y, width) {
    ...
    if (baseAp === maxAp) {
    var text = '\\c[17]\\fiMastered\\fi\\c[0]';
    this.drawTextEx(text, x + 87, y);
    } else {
    var text = baseAp + "/" + maxAp;
    this.drawText(text, x, y, width, 'right');
    }
